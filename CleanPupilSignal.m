...
%       If Method is left blank, will assume spline
%   moving_window_fntn is default 'median' for using movmedian function, or
...
for iteration = 1:size(vector,1)
    ...
    if isnan(vector(iteration)) == 0 %if pupil size is not NaN
...
if isnan(remove_prop) %if remove_perc not given, don't do anything
else %if remove_perc given, make NaN if the proportion missing is above
...
if isnan(moving_window_fntn) || moving_window_fntn=="median"
